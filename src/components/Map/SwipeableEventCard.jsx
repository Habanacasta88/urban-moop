...
export const SwipeableEventCard = ({ event, distance, onNext, onPrev, onClick }) => {
    ...
    return (
        <div className="absolute bottom-24 left-0 right-0 px-4 z-20 flex justify-center perspective-1000">
            ...
            >
                {/* Micro Label */}
                <div className="absolute top-0 right-0 left-0 h-1 bg-gradient-to-r from-brand-500 to-brand-300 opacity-20" />
                ...
                <div className="flex h-36">
                    ...
                    >
                        ...
                        {event.type === 'moop' && (
                            <div className="absolute top-2 left-2 bg-brand-400 text-white text-[10px] font-black px-2 py-0.5 rounded shadow-sm flex items-center gap-1">
                                <Users size={10} /> MOOP
                            </div>
                        )}
                        {/* Live Indicator */}
                    ...
                    <div className="w-2/3 p-4 flex flex-col justify-between relative bg-white">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-1 mb-1">
                                <span className="text-[9px] font-bold text-brand-700 bg-brand-50 px-2 py-0.5 rounded-md border border-brand-100 uppercase tracking-wide flex items-center gap-1">
                                    ✨ Recomendado ahora
                                </span>
                            </div>

                            <h3 className="font-excep font-black text-text text-lg leading-tight line-clamp-2 mb-1">
                                {event.title}
                            </h3>
                            <div className="flex items-center gap-1 text-xs text-muted font-medium">
                                ...
                                <span className="truncate">
                                    ...
                                    {distance && <span className="text-muted font-normal ml-1">· {distance}</span>}
                                </span>
                            </div>
                        </div>
                            ...
                            <div className="flex items-center gap-3 absolute left-0 bottom-0.5">
                                {event.attendees && (
                                    <div className="flex items-center gap-1 bg-gray-50 px-2 py-1 rounded-lg border border-border">
                                        ...
                                        <span className="text-xs font-bold text-text-2">{event.attendees}</span>
                                    </div>
                                )}
                                {event.endsIn && (
